...
class CalcViewModel: ViewModel() {
    ...
    fun getPaymentPerMonth()  {
        ...
        _monthPay.value = String.format(Locale.ROOT, "%.02f",topPart/bottomPart).toDouble()
        //_monthPay.value = topPart/bottomPart
    ...
    fun getGrafik() {
        ...
        for(i in 1.rangeTo(_month.value!!)) {
            ...
            graf.month = i
            graf.payPerMonth = mm
            graf.sumPercent = String.format(Locale.ROOT,"%.02f", dolg!!*monthRate).toDouble()
            graf.sumDolg = String.format(Locale.ROOT,"%.02f", mm-dolg!!*monthRate).toDouble()
            dolg = String.format(Locale.ROOT,"%.02f", dolg!! - (mm-dolg!!*monthRate)).toDouble()
